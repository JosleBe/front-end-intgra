...
const RegisterCampaign = () => {
    ...
    const [nombre, setNombre] = useState('Utiles escolares');
    const [descripcion, setDescripcion] = useState('Añadir una descripción');
    ...
    const handleRegister = async () => {
        ...
        // Asegurarse de que las fechas sean válidas y que la fecha de inicio sea antes que la fecha de fin
        const fechaInicioDate = new Date(fechaInicio);
        const fechaFinDate = new Date(fechaFin);
        if (isNaN(fechaInicioDate.getTime()) || isNaN(fechaFinDate.getTime())) {
            return setError("Las fechas deben ser válidas.");
        }
        if (fechaInicioDate >= fechaFinDate) {
            return setError("La fecha de inicio debe ser menor que la fecha de fin.");
        }
    
        // Validación de la subida de imagen (asegurarse de que la imagen esté en base64)
        if (!image) return setError("La imagen es obligatoria.");
    
        // Validación de la categoría (asegurarse de que se haya seleccionado una categoría)
        if (!categoria) return setError("Debe seleccionar una categoría.");
    
        // Validación de la plantilla seleccionada (asegurarse de que se haya seleccionado una plantilla)
        if (!plantillaSeleccionada) return setError("Debe seleccionar una plantilla.");
    
        // Validación del recurso (asegurarse de que se haya seleccionado un recurso)
        if (!recursoTipo) return setError("Debe seleccionar un recurso.");
    
        // Validación de la ubicación seleccionada
        if (!selectedLocation || !selectedLocation.address) return setError("Debe seleccionar una ubicación en el mapa.");
    
        // Crear el objeto que se enviará en JSON
        const data = {
            alignment,
            ...
            fechaInicio,
            fechaFin,
            meta: Number(meta), // Asegurarse de que 'meta' sea un número
            lugar,
            location: selectedLocation, // Directamente el objeto location (sin necesidad de stringify)
            recursoTipo,
            cantidad,
            plantillaSeleccionada // Directamente el objeto o array (sin necesidad de stringify)
        };
        ...
        try {
            // Enviar la información al servidor usando JSON
            ...
            if (response.status === 201) {
                message.success("Campaña registrada correctamente");
            ...
            message.error("Hubo un problema al registrar la campaña.");
        }
    };
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <FormControl fullWidth>
                            ...
                            <Input
                                ...
                                placeholder="Ej. Campaña solidaria"
                            />
                        </FormControl>
